...
bool isFavoriteTeam(const std::string& team_id,
  ...
  for (size_t i = 0; i < favorite_count; i++) {
    if (team_id == favorite_team_ids[i]) {
      return true;
    }
  }
...
RefreshPlan makePlan(ActivityLevel activity,
                     uint8_t retry_attempt = 0) {
  RefreshPlan plan;
  plan.activity = activity;
  plan.interval_ms =
      refreshIntervalMs(activity, retry_attempt);
...
RefreshPlan planRefresh(const Game* games,
                        size_t game_count,
                        const std::string* favorite_team_ids,
                        size_t favorite_count,
                        int64_t now_unix,
  ...
  for (size_t i = 0; i < game_count; i++) {
    ...
    if (isLive(game.status) &&
        includesFavorite(
            ...
            favorite_count)) {
      return makePlan(
          ActivityLevel::kLiveFavoriteGame);
    }
  }

  for (size_t i = 0; i < game_count; i++) {
    if (isLive(games[i].status)) {
      return makePlan(
          ActivityLevel::kOtherLiveGame);
    }
  }
  ...
  if (has_game_today) {
    return makePlan(ActivityLevel::kGamesToday);
  }
